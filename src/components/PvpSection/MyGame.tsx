...
export function MyGame() {
  ...
  const [bets, setBets] = useState<GameMatch[]>([]);
  ...
  useWatchContractEvent({
    ...
    onLogs(logs) {
      const newEvents = logs.map((log) => {
        const args = log.args as unknown as AllBetsEvent;
        return {
          betId: args.betId,
      ...
      try {
        const formattedBets = (data as GameMatch[]).map((bet) => {
          const winner = bet.fulfilled 
            ? (bet.player1Face === bet.outcome ? bet.player1 : bet.player2)
            : undefined;
        ...
        const p2Bets = formattedBets.filter(bet => bet.player2 && bet.player2 === address);
  ...
  const tabTitle = activeTab === 'player1' ? 'My Created Bets' : 'My Joined Bets';
          ...
          {activeBets.length === 0 ? (
            ...
            <table className="w-full whitespace-nowrap">
              <thead className="bg-gray-100 text-gray-600">
                <tr>
                  {['Bet ID', 'Player 1', 'Player 2', 'Amount', 'Token', 'Choice', 'Status', 'Outcome', 'Winner'].map((header) => (
                ...
                {activeBets.map((bet) => (
                  <tr key={bet.id.toString()} 
                      className={`hover:bg-gray-50 ${
                        bet.status.toLowerCase() === 'fulfilled' ? 'bg-green-50' :
                        bet.status.toLowerCase() === 'pending' ? 'bg-yellow-50' : ''
                      }`}>
                    <td className="px-4 py-4">{bet.betId.toString()}</td>
                    <td className="px-4 py-4">
                      {bet.player1 === address ? "You" : shortenAddress(bet.player1)}
                    </td>
                    <td className="px-4 py-4">
                      {bet.player2 
                        ? (bet.player2 === address ? "You" : shortenAddress(bet.player2))
                        : "Waiting..."}
                    </td>
                    <td className="px-4 py-4">{formatEther(bet.amount)}</td>
                    <td className="px-4 py-4">{getTokenSymbol(bet.token)}</td>
                    <td className="px-4 py-4">{bet.player1Face ? "Heads" : "Tails"}</td>
                    <td className="px-4 py-4">{bet.status}</td>
                    <td className="px-4 py-4">
                      {bet.fulfilled ? 
                        (bet.outcome ? "Heads" : "Tails") : 
                        "Pending"}
                    </td>
                    <td className="px-4 py-4">
                      {bet.winner ? (
                        <div className={bet.winner === address ? "text-green-600 font-bold" : ""}>
                          {bet.winner === address ? "You won!" : `${shortenAddress(bet.winner)} won`}
                          {bet.winner === address && bet.paid > 0 && (
                            <div className="text-sm text-gray-500">
                              (+{formatEther(bet.paid)} {getTokenSymbol(bet.token)})
                            </div>
                          )}
                        </div>
              ...
              {events.slice(0, 5).map((event, index) => (
                ...
                >
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p><strong>Bet ID:</strong> {event.betId.toString()}</p>
                    <p><strong>Match ID:</strong> {event.id.toString()}</p>
                    <p><strong>Status:</strong> {event.status}</p>
                    <p><strong>Player1:</strong> {shortenAddress(event.player1)}</p>
                    <p>
                      <strong>Player2:</strong> {event.player2 
                        ? shortenAddress(event.player2) 
                        : "None"}
                    </p>
                    <p>
                      <strong>Amount:</strong> {formatEther(event.amount)} {getTokenSymbol(event.token)}
                    </p>
                    {event.outcome !== undefined && (
                      <p><strong>Outcome:</strong> {event.outcome ? "Heads" : "Tails"}</p>
                    )}
                    {event.winner && event.winner !== "0x0000000000000000000000000000000000000000" ? (
                      <p 
                        className={
                      ...
                      >
                        <strong>Winner:</strong> {event.winner === address ? "You" : shortenAddress(event.winner)}
                        {event.payout > 0 && (
                          <span> (Payout: {formatEther(event.payout)} {getTokenSymbol(event.token)})</span>
                        )}
                      </p>
