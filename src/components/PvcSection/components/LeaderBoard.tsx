...
import { Trophy, ArrowLeftCircle, ArrowRightCircle, Award, Users, User, List } from 'lucide-react';
...
const TOKEN_BETS_QUERY = gql`
  query TokenBets($skip: Int!, $token: Bytes!) {
    gameWons(first: 1000, skip: $skip, orderBy: payout, orderDirection: desc, where: { token: $token }) {
      player
      token
      payout
    }
...
async function fetchTopPlayersForToken(selectedToken: string): Promise<TopPlayersByToken> {
  let allBets: Bet[] = [];
  let skip = 0;
  const pageSize = 1000;
  ...
  const aggregates = allBets.reduce((acc: { [key: string]: AggregatedPlayer }, bet) => {
...
async function fetchAllBets(setAllBets: React.Dispatch<React.SetStateAction<Bet[]>>): Promise<Bet[]> {
  let allBets: Bet[] = [];
  let skip = 0;
  const pageSize = 1000;
...
const formatPayout = (payout: string): string => (Number(payout) / 1e18).toFixed(2);
  ...
  return (
    <div className="leaderboard p-4 max-w-6xl mx-auto">
      ...
      <div className="flex border-b mb-4">
        <button
          className={`px-4 py-2 font-medium flex items-center ${activeTab === 'all-bets' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
        ...
        <button
          className={`px-4 py-2 font-medium flex items-center ${activeTab === 'token-leaderboard' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
      ...
      {activeTab === 'token-leaderboard' && (
        <div className="mb-4">
          <label htmlFor="token-select" className="mr-2 font-medium">Select Token:</label>
          <select
            id="token-select"
            value={selectedToken}
      ...
      {!loading && !error && (
        <div className="tab-content">
                ...
                <table className="min-w-full bg-white border">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="py-2 px-4 border-b text-left">Player Choice</th>
                  ...
                  <tbody>
                    {paginatedBets.map((bet, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                        <td className="py-2 px-4 border-b">{truncateAddress(bet.player)}</td>
                        <td className="py-2 px-4 border-b">{bet.playerChoice ? 'Tails' : 'Heads'}</td>
                        <td className="py-2 px-4 border-b">{bet.outcome ? 'Tails' : 'Heads'}</td>
                        <td className="py-2 px-4 border-b">{(parseFloat(bet.profit) / 1e18).toFixed(2)}</td>
                        <td className={`py-2 px-4 border-b ${bet.userWon ? 'text-green-500' : 'text-red-500'}`}>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-700">
                    ...
                    <span className="font-medium">{Math.min(currentPage * itemsPerPage, allBets.length)}</span> of{' '}
                    <span className="font-medium">{allBets.length}</span> results
                  </div>
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      className={`flex items-center p-2 border rounded ${
                        currentPage === totalPages ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-blue-500 hover:bg-blue-50'
                      }`}
                    >
                      Next
          ...
          {activeTab === 'token-leaderboard' && selectedToken && topPlayers && Object.keys(topPlayers).length > 0 && (
            <div>
                    ...
                    <table className="min-w-full bg-white border">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="py-2 px-4 border-b text-left">Player</th>
                          <th className="py-2 px-4 border-b text-left">Total Payout</th>
                        </tr>
                      </thead>
                      <tbody>
                        {players.map((player, index) => (
                          <tr key={player.player} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                            <td className="py-2 px-4 border-b">
                              <div className="flex items-center">
                                {index < 3 ? (
                                  <Award className={`h-5 w-5 mr-1 ${getMedalColor(index)}`} />
                                ) : (
                                  <span className="text-gray-500 w-5 mr-1 text-center">{index + 1}</span>
                                )}
                              </div>
                            </td>
                ...
                <tbody>
                  {playerBets.map((bet, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                      ...
                      <td className="py-2 px-4 border-b">{bet.outcome ? 'Tails' : 'Heads'}</td>
                      <td className="py-2 px-4 border-b">{(parseFloat(bet.profit) / 1e18).toFixed(2)}</td>
                      <td className={`py-2 px-4 border-b ${bet.userWon ? 'text-green-500' : 'text-red-500'} flex items-center`}>
                        {bet.userWon ? (
                          <>
          ...
          {activeTab === 'token-leaderboard' && (!selectedToken || !topPlayers || Object.keys(topPlayers).length === 0) && !loading && (
            <div className="text-center py-4">{selectedToken ? 'No leaderboard data available for this token.' : 'Please select a token to view the leaderboard.'}</div>
          )}
